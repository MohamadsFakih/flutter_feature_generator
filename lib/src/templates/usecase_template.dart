...
class UseCaseTemplate {
  static String generateUseCases(String featureName, List<ApiEndpoint> endpoints) {
    ...
    // Generate methods
    for (final endpoint in endpoints) {
      methods.add(_generateUseCaseMethod(endpoint, featureName));
    }
...
@injectable
class $className {
  final $repositoryClassName repository;
...
''';
  ...
  static String _generateUseCaseMethod(ApiEndpoint endpoint, String featureName) {
    ...
    final repositoryCall = _generateRepositoryCall(endpoint, featureName);
    
    return '''  Future<Either<Error, $returnType>> $methodName($parameters) =>
      repository.$repositoryCall;''';
  }

  static String _generateRepositoryCall(ApiEndpoint endpoint, String featureName) {
    final methodName = _generateMethodName(endpoint);
